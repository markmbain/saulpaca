...
def init_openai():
    ...
    # Default logprobs to None if not provided, and make sure it's an integer
    logprobs = os.getenv("LOGPROBS")
    logprobs = int(logprobs) if logprobs is not None else None

    # Default headers to an empty dictionary if not provided
    default_headers = os.getenv("DEFAULT_HEADERS")
    default_headers = eval(default_headers) if default_headers is not None else {}
    ...
    config = {
        ...
        "logprobs": logprobs,  # Add logprobs to the configuration
        "default_headers": default_headers  # Ensure headers are a valid dictionary
    }
    ...
    embed_config = {
        "model": os.getenv("EMBEDDING_MODEL_OPENAI"),
